...
The first profile uses a 32-byte random `stream_id` generated by the sender. The `stream` tag value is
`stream_id_hex`, the lowercase hex encoding of those 32 bytes. The MLS-delivered start event id is the durable anchor.
...
`final-kind`. For `stream-type=text`, the first profile requires `final-kind=9`.
...
For the first text profile, the final message `stream` tag MUST match the start event's `stream` tag exactly.
`stream-start` is the lowercase hex Nostr event id of the kind `1200` start event. `stream-hash` is the lowercase hex
encoding of the 32-byte transcript hash. `stream-chunks` is the unsigned decimal chunk count. The final message content
is the complete final text.
...
`component_secret` is the 32-byte `SafeExportSecret(0x8006)` output for the MLS epoch that authenticated the start
event. Implementations MUST reject any other component-secret length for this profile.

`AgentTextStreamKeyContextV1` uses Marmot canonical encoding:

```text
struct {
  opaque version<1..255>;
...
`version` is the UTF-8 string `v1`. `stream_id` is the raw 32-byte value named by the start event's `stream` tag, not
the hex string. `start_event_id` is the 32-byte Nostr event id of the MLS-delivered start app payload.
