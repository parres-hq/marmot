...
**Required fields:**
- **`content`**: Serialized `MLSMessage` containing the MLS `Welcome` object. New implementations MUST use base64 encoding.
...
The `content` field contains an encoded MLS `Welcome` object. The encoding format (base64 or hex) is specified by the `encoding` tag. The Welcome object includes:
