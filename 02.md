...
- **`encoding`**: **Required**. Specifies the encoding format of the `content` field. MUST be `["encoding", "base64"]`. Implementations MUST reject events with missing or unrecognized encoding tags. Hex encoding was used by early implementations but is no longer supported and MUST be rejected.
...
The `content` field contains a base64-encoded MLS `Welcome` object. The Welcome object includes:
