...
Welcome Events use [NIP-59](https://github.com/nostr-protocol/nips/blob/master/59.md) gift-wrapping for privacy and unlinkability. The top-level event published to relays is a `kind: 1059` gift wrap containing a `kind: 13` seal, which in turn contains the unsigned inner `kind: 444` Welcome rumor described below.

### Layered Structure

1. **Rumor (`kind: 444`)**: An unsigned inner event carrying the Welcome payload
2. **Seal (`kind: 13`)**: A signed NIP-59 seal containing the encrypted rumor
3. **Gift Wrap (`kind: 1059`)**: The relayed outer event addressed to the recipient

## Inner Welcome Rumor Structure
...
```json
{
  ...
  "pubkey": "a1633cafe37eeebe2b39b4ec5f3d74c35e61fa7e7e6b7b8c5f7c4f3b2a1b2c3d",
...
**Important**: This inner `kind: 444` rumor is NOT signed (`"sig"` field omitted) to prevent accidental public publishing if it leaks. The enclosing `kind: 13` seal and `kind: 1059` gift wrap are signed per NIP-59.

### Field Details

**Required fields:**
- **`content`**: Serialized `MLSMessage` whose `wire_format` is `mls_welcome`, carrying the MLS `Welcome` object. The serialized bytes are base64-encoded.
...
### MLSMessage with wire_format mls_welcome

The `content` field contains a base64-encoded **MLSMessage**. The MLSMessage MUST have **wire_format** = **mls_welcome**; in that case the message payload is an MLS `Welcome` object. The encoding of the serialized MLSMessage (base64) is specified by the `encoding` tag. The Welcome payload includes:
...
When receiving a gift-wrapped Welcome Event and, after NIP-59 unwrapping, obtaining the inner `kind: 444` rumor, clients MUST:
...
7. **Catch up, then self-update**: Make a best-effort attempt to process outstanding Commits for the group, then create and commit an Update proposal for your own LeafNode as soon as practical (see below)

### Post-Join Self-Update Requirement

After successfully processing a Welcome message, clients SHOULD perform a self-update (MLS Update proposal for their own LeafNode) as soon as possible. Before sending that self-update, clients SHOULD make a best-effort attempt to process outstanding Commits from configured relays so the self-update is based on the freshest available group state. Sending a self-update from stale state increases the risk of local state divergence (effectively "forking yourself" out of the group).
...
If a client cannot process all outstanding Commits immediately (for example, relay unavailability), it SHOULD continue catch-up attempts and avoid sending application messages until it confirms it is on the current group epoch.
...
1. **MLSMessage (wire_format mls_welcome)**: The event content is an MLSMessage carrying a Welcome object with all cryptographic material for group access
