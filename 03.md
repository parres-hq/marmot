...
1. **CRITICAL**: **Derive encryption key**: Use the MLS [Exporter](https://www.rfc-editor.org/rfc/rfc9420.html#section-8.5) to derive a 32-byte encryption key directly:
   ```text
   encryption_key = MLS-Exporter("marmot", "group-event", 32)
   ```
2. **CRITICAL**: **Generate nonce**: Generate a cryptographically random 12-byte nonce for each event.
3. **CRITICAL**: **Encrypt**: Encrypt the serialized `MLSMessage` bytes using ChaCha20-Poly1305:
   ```text
   nonce             = Random(12)
   ciphertext        = ChaCha20-Poly1305.encrypt(encryption_key, nonce, mlsmessage_bytes)
...
- **CRITICAL**: ChaCha20-Poly1305 encryption with key derived via `MLS-Exporter("marmot", "group-event", 32)` and random nonce per event
