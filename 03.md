...
|-------|-------------|-------------------|
| `content` | Serialized [`MLSMessage`](https://www.rfc-editor.org/rfc/rfc9420.html#section-6-4) encrypted with ChaCha20-Poly1305. `content` MUST be `base64(nonce \|\| ciphertext)` where `ciphertext` is the full output of `ChaCha20-Poly1305.encrypt`. | Base64 (standard, with padding). |
...
- **AEAD authentication failure**: If `ChaCha20-Poly1305.decrypt` returns an authentication error, the implementation MUST treat this as fatal for that event: log the authentication failure with the event ID and `nostr_group_id`, drop the event, and MUST NOT process or expose any portion of the unauthenticated plaintext.
