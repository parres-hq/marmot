...
1. **Derive encryption key**: Use the MLS [Exporter](https://www.rfc-editor.org/rfc/rfc9420.html#section-8.5) to derive a 32-byte secret, then expand it into an encryption key:
   ```
...
3. **Encrypt**: Encrypt the serialized `MLSMessage` bytes using ChaCha20-Poly1305. The `nostr_group_id` (the `h` tag value, as raw 32 bytes) is included as AAD:
   ```
   nonce             = Random(12)
...
- ChaCha20-Poly1305 encryption with key derived via `MLS-Exporter` + `HKDF-Expand` and random nonce per event
