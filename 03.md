...
- Before applying a Commit containing SelfRemove proposals, clients MUST verify that the resulting `admin_pubkeys` set would be non-empty. If the Commit would remove all remaining admins, it MUST be rejected at the application layer — the group would become unmanageable. Members MUST transfer admin rights before leaving
...
- Admin depletion prevention (a Commit containing SelfRemove proposals MUST be rejected if it would leave `admin_pubkeys` empty)
- **CRITICAL**: group-event outer encryption using `group_event_key = MLS-Exporter("marmot", "group-event", 32)`, empty-byte-string AAD, and random nonce per event
