...
Marmot therefore treats Commit creation and processing as a convergence problem as well as a cryptographic one. A Commit may be cryptographically valid while still being unsafe to build on if the local client view is stale or branch-uncertain.

### Pre-Commit Freshness Requirement

Before creating any Commit, clients MUST make a bounded best-effort attempt to process newer visible group state from the authenticated relay set.

For all Commits, clients SHOULD:

1. query all configured relays that are currently reachable,
2. fetch unseen `kind: 445` events for the group,
3. process all valid and processable Commits and relevant Proposals,
4. stop when no additional processable events remain or when the implementation's timeout budget expires.

For structural Commits (member add, member remove, or group extension changes), clients MUST defer Commit creation if local state is known to be branch-uncertain or stale relative to visible network state.

For self-update Commits, clients SHOULD also perform the same bounded catch-up. A self-update Commit is less dangerous than a structural Commit, but creating one from stale state can still fork the sender out of the group.

When a Commit adds one or more members, implementations SHOULD compute the MLS `epoch_authenticator` for the resulting Welcome epoch and include it in encrypted Welcome-associated metadata for each new member. This does not replace MLS validation; it provides an additional same-epoch branch confirmation signal for mixed-client or adversarial delivery conditions.
...
If the Commit adds one or more members, the sender MAY place the parent Commit event id in the private Welcome rumor as a diagnostic hint. If used, this hint MUST remain private to the rumor and MUST NOT be exposed on public outer metadata.
...
### Uncertain Local State and Drift

Clients SHOULD maintain some local indication that branch convergence is not yet trusted. This is a local operational condition, not an MLS wire-level concept.

Clients SHOULD enter this uncertain local condition when one or more of the following occurs:

- multiple competing Commits are observed for the same epoch and the winner is not yet established locally,
- a Welcome branch cannot be reconciled with the observed Commit history,
- the client observes evidence that its local epoch is stale relative to visible relay state,
- ratchet tree validation or other MLS validation fails in a way that suggests divergent or poisoned state rather than random malformed input,
- a previously accepted branch is later determined to be losing according to Marmot's Commit conflict rules.

While in this uncertain local condition, clients MUST:

1. suspend outbound application messages,
2. avoid creating structural Commits,
3. attempt bounded catch-up and recovery,
4. quarantine local branch state if it is determined to be invalid or losing.

### Same-Epoch Conflict Handling

The same-epoch Commit selection rule above determines which visible branch wins. However, clients MUST NOT interpret that rule as permission to immediately act on incomplete visibility.

When same-epoch competing Commits are observed, clients SHOULD:

1. enter the uncertain local condition if they cannot yet determine the winning branch locally,
2. defer structural Commits while uncertainty remains,
3. continue bounded catch-up until the winner is determined or retry budget is exhausted,
4. quarantine any local join state or pending Welcome branch that depends on a losing Commit.

If a Welcome is later found to depend on a losing same-epoch Commit, the resulting joined state MUST NOT be activated for messaging and SHOULD require re-invitation or manual recovery.

### Welcome-Epoch Confirmation Guidance

Marmot's preferred additional consistency signal is the MLS `epoch_authenticator` for the Welcome epoch:

1. the inviter computes the Welcome epoch's `epoch_authenticator`,
2. the inviter includes it only inside encrypted Welcome-associated metadata,
3. the joiner derives its own local `epoch_authenticator` after successful Welcome processing,
4. the joiner compares the two before first outbound traffic.

If present and mismatched, this condition indicates same-epoch branch divergence, mixed-client inconsistency, or other drift. Clients MUST treat it as an uncertain local state and quarantine the joined branch.

Marmot implementations SHOULD support this check. In mixed-client environments, failing to perform this confirmation materially increases the risk that cryptographically valid but divergent branches remain undetected long enough to poison future Welcomes or self-updates.
...
### Sending Restrictions Under Uncertainty

Clients MUST NOT send application messages when local group state is known to be uncertain, including immediately after joining if the client has not yet completed bounded best-effort sync described in [MIP-02](02.md).

This restriction is necessary because application traffic sent from stale or losing branches can amplify drift and make recovery harder even when MLS cryptographic validation still succeeds locally.
...
- Bounded catch-up before Commit creation
- Structural Commit deferral while state is uncertain
- Send suspension and recovery behavior for uncertain local state
