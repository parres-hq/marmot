...
1. **Derive group-event outer encryption key**: Use the MLS [Exporter](https://www.rfc-editor.org/rfc/rfc9420.html#section-8.5) to derive the per-epoch 32-byte key used for Marmot's outer group-event encryption:

   ```text
   group_event_key = MLS-Exporter("marmot", "group-event", 32)
   ```

2. **Generate nonce**: Generate a cryptographically random 12-byte nonce for each event.
3. **Encrypt**: Encrypt the serialized `MLSMessage` bytes using ChaCha20-Poly1305 with canonical AAD handling:

   ```text
   nonce             = Random(12)
   aad               = b"" (empty byte string)
   ciphertext        = ChaCha20-Poly1305.encrypt(key=group_event_key, nonce=nonce, plaintext=mlsmessage_bytes, aad=aad)
   event.content     = base64(nonce || ciphertext)
   ```

4. **Publish**: Publish the event to relays using a fresh ephemeral Nostr keypair. Generate a new keypair for every event — never reuse.
...
**Key lifetime**: `group_event_key` is stable for the entire MLS epoch. Every event in the same epoch uses the same derived key, so nonce uniqueness is critical. A new key is derived automatically when a Commit advances the group to a new epoch.
...
**Exception for MIP-06**: When `sender_type` is `new_member_commit` and `marmot_multi_device` (`0xF2F0`) is present in `GroupContext.extensions`, clients MUST apply the External Commit authorization rules in [MIP-06](06.md) instead of rejecting solely because the sender is not an admin.
...
- In groups using [MIP-06](06.md), SelfRemove is leaf-scoped: it removes only the sending device's leaf, not any same-identity sibling leaves belonging to the same Nostr pubkey. Clients SHOULD warn users when other same-identity leaves remain and MUST NOT present a leaf-scoped SelfRemove as leaving the group entirely unless no same-identity leaves remain.
...
- For Commit messages that are not MIP-06 External Commits, `FramedContent.authenticated_data` MUST be the empty byte string unless another Marmot specification explicitly defines a non-empty value
