...
1. **CRITICAL**: **Derive exporter secret**: Use the MLS [Exporter](https://www.rfc-editor.org/rfc/rfc9420.html#section-8.5) to derive a per-epoch exporter secret:
   ```text
   exporter_secret = MLS-Exporter("marmot", "group-event", 32)
   ```
2. **CRITICAL**: **Derive encryption key**: Derive the 32-byte ChaCha20-Poly1305 key from the exporter secret:
   ```text
   encryption_key = HKDF-Expand(exporter_secret, "mip03-group-event-v1", 32)
   ```
   `HKDF-Expand` in this step MUST use SHA-256 as the underlying hash function.
3. **CRITICAL**: **Generate nonce**: Generate a cryptographically random 12-byte nonce for each event.
4. **CRITICAL**: **Encrypt**: Encrypt the serialized `MLSMessage` bytes using ChaCha20-Poly1305 with canonical AAD handling:
   ```text
   nonce             = Random(12)
   ...
   ```
5. **CRITICAL**: **Publish**: Publish the event to relays using a fresh ephemeral Nostr keypair. Generate a new keypair for every event — never reuse.
...
- **RNG failure**: If the cryptographically secure random number generator fails or returns insufficient entropy for nonce generation, the encryption operation MUST be aborted. Implementations MUST NOT fall back to a deterministic or low-entropy nonce.
...
- **CRITICAL**: ChaCha20-Poly1305 encryption with `encryption_key = HKDF-Expand(MLS-Exporter("marmot", "group-event", 32), "mip03-group-event-v1", 32)`, empty-byte-string AAD, and random nonce per event
