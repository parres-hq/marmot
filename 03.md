...
If the Commit adds one or more members, the sender MUST place the parent Commit event id in the private Welcome rumor so the recipient can bind the Welcome to its intended branch without exposing that linkage publicly.
...
### State Uncertainty and Drift

Clients SHOULD maintain a local `StateUncertain` condition for groups where branch convergence is not yet trusted. This is a local operational state, not an MLS wire-level concept.

Clients SHOULD enter `StateUncertain` when one or more of the following occurs:
...
While in `StateUncertain`, clients MUST:
...
1. enter `StateUncertain` if they cannot yet determine the winning branch locally,
...
Clients MUST NOT send application messages when local group state is known to be uncertain, including while in `PendingJoin` or `StateUncertain` as described in [MIP-02](02.md).
...
- Send suspension and recovery behavior for `StateUncertain`
