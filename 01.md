...
- [`required_capabilities`](https://docs.rs/openmls/latest/openmls/extensions/struct.RequiredCapabilitiesExtension.html) - Defines what MLS features all members must support. The group's `required_capabilities` MUST include `self_remove` (`0x000a`) as a required proposal type, in addition to any required extensions. This ensures the group is "self-remove-capable" per the [MLS Extensions draft](https://datatracker.ietf.org/doc/draft-ietf-mls-extensions/). **Backwards compatibility**: Members whose KeyPackages do not advertise `self_remove` support cannot be added to groups that require it (MLS enforces capability matching). Existing groups created without `self_remove` in their `required_capabilities` cannot use SelfRemove and would need to be recreated.
...
Groups that enable multi-device External Commit behavior per [MIP-06](06.md) MUST additionally include the `marmot_multi_device` extension (`0xF2F0`) in `GroupContext.extensions`, include `0xF2F0` in `required_capabilities`, and ensure all members advertise support in `LeafNode.capabilities.extensions`.
...
2. **Commit**: Changes are applied to the group state. Admins can commit any proposals, while non-admin members can only create self-update Commits and dedicated SelfRemove-only Commits. See [MIP-03](03.md) for details.
...
**CRITICAL**: This extension MUST use TLS presentation language notation ([RFC 8446 Section 3](https://www.rfc-editor.org/rfc/rfc8446.html#section-3)) to describe field structure. In Marmot, all variable-length vectors use **QUIC-style variable-length integer encoding** ([RFC 9000 Section 16](https://www.rfc-editor.org/rfc/rfc9000.html#section-16)) for length prefixes, as implemented by the `tls_codec` crate (v0.4+). This is Marmot's wire-format convention; implementations MUST NOT substitute RFC 8446 fixed-width vector length prefixes for these fields. This means length prefix size varies based on the content length:
...
**Updates**: Any field in this extension can be updated through MLS Proposal/Commit mechanisms.

- All extension updates require admin authorization
- Before processing any Commit that modifies this extension, implementations MUST verify the committer is listed in the current `admin_pubkeys` array
- Self-update Commits and SelfRemove-only Commits do not modify this extension and do not require admin authorization (see [MIP-03](03.md))
- **Admin SelfRemove restriction**: A member whose public key is in `admin_pubkeys` MUST NOT send a SelfRemove proposal. The member MUST first be removed from `admin_pubkeys` via a GroupContextExtensions update before departing through the SelfRemove path. An admin can commit this update themselves (self-demotion) unless they are the last admin, in which case another admin MUST be designated first. This ensures the admin list is always updated through the admin authorization path. See [MIP-03](03.md) for the complete admin departure flow and validation requirements
