...
The `admin_pubkeys` and `relays` fields are TLS vectors of individually length-prefixed elements following TLS presentation language (`<0..2^16-1>`). Both the outer vector and each inner element use 2-byte (uint16) length prefixes. Each element carries its own 2-byte length prefix followed by its byte content.

**Admin Public Keys Array**:
Each element in the `admin_pubkeys` vector is a 2-byte length-prefixed `AdminPubkey`:
- Length prefix: 2-byte uint16 encoding the byte length of this element
...
**Relays Array**:
Each element in the `relays` vector is a 2-byte length-prefixed `RelayUrl`:
- Length prefix: 2-byte uint16 encoding the byte length of this element
...
```text
[2-byte outer length][2-byte len₁][64 hex chars of pubkey₁][2-byte len₂][64 hex chars of pubkey₂]
```
...
**Forward Compatibility**: The extension format is append-only: future versions may only add new fields at the end, and parsers MUST parse fields in the documented order. When encountering unknown future versions:
