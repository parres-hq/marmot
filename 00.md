...
The `mls_proposals` tag includes self_remove (`0x000a`) which MUST be included. Note: `self_remove` and `last_resort` share the same numeric value (`0x000a`) but belong to different IANA registries (MLS Proposal Types vs MLS Extension Types), so there is no conflict.

### Field Explanations

**Required fields:**
- **`d`**: A randomly generated identifier for this KeyPackage slot. Clients SHOULD generate a cryptographically random 32-byte hex string as the `d` tag value when first publishing a KeyPackage. When rotating a KeyPackage, the client publishes a new `kind:30443` event with the same `d` tag value. On relays that receive the newer publish, it replaces the prior event for that `(kind, pubkey, d)` slot. Clients publishing successive rotations for the same slot MUST use a strictly increasing `created_at` value; if the wall clock has not advanced, they MUST increment `created_at` locally before publishing to preserve a total order for that slot. Clients MAY generate a new `d` tag value when they wish to maintain multiple independent KeyPackage slots simultaneously (e.g., one per device). The `d` tag value MUST NOT be empty.
...
- **`mls_extensions`**: Array of supported non-default MLS extension IDs (see [MLS spec](https://www.rfc-editor.org/rfc/rfc9420.html#name-extensions)). Default extensions MUST NOT be listed. This signals **individual client capabilities** via `LeafNode.capabilities.extensions` in KeyPackages. Marmot implementations MUST include the `0xf2ee` extension for marmot_group_data and the `0x000a` extension for last_resort. Implementations that support [MIP-06](06.md) MUST additionally include `0xf2f0` (marmot_multi_device).
- **`mls_proposals`**: Array of supported non-default MLS proposal type IDs (see [MLS Extensions](https://datatracker.ietf.org/doc/draft-ietf-mls-extensions/)). Default proposal types MUST NOT be listed. This signals **individual client capabilities** via `LeafNode.capabilities.proposals` in KeyPackages. Marmot implementations MUST include `0x000a` for self_remove (see [MIP-03](03.md)).
- **`relays`**: Relay URLs where this KeyPackage is published. MUST contain at least one valid WebSocket URL (ws:// or wss://). Used for KeyPackage discovery and deletion. Welcome events are delivered via the recipient's inbox relays, not via this tag.
...
- **`-`**: Ensures only the author can publish this event (see [NIP-70](https://github.com/nostr-protocol/nips/blob/master/70.md)). Note: Implementations SHOULD omit it unless publishing to relays known to support NIP-70 + NIP-42.
...
To fetch all current KeyPackages for a user (one per `d` tag slot, deduplicated by relays):

```typescript
const filter = {
  kinds: [30443],
  authors: ["f1e2d3c4b5a697887766554433221100ffeeddccbbaa99887766554433221100"]
};
```

#### Selecting a KeyPackage for Invitation

An `authors` query can return multiple current `kind:30443` events for the same user, typically one per active `d` tag slot. Before using one to send a Welcome, inviters MUST apply the following selection policy:

1. Reject invalid candidates (credential mismatch, invalid encoding, invalid ciphersuite, malformed `relays` tag, invalid `i` tag, or any other validation failure in this MIP)
2. Prefer non-`last_resort` candidates; if any valid candidate omits the `last_resort` extension, exclude all `last_resort` candidates from consideration
3. Among the remaining candidates, prefer the one with the highest `created_at`
4. If multiple candidates still tie for highest `created_at`, any of them is acceptable and the inviter MAY choose arbitrarily

If the inviter is already aware that a candidate was explicitly decommissioned (for example, because it has already received a valid NIP-09 deletion event from the same author), it SHOULD exclude that candidate from consideration. If all candidates are rejected, the invitation cannot proceed. The inviter SHOULD surface an error and wait for the invitee to publish a fresh KeyPackage.
...
`kind:30443` is an addressable event. For each combination of `pubkey`, `kind`, and `d` tag value, a relay stores only the most recent version it has seen. Publishing a new `kind:30443` event under the same `d` tag value replaces the prior event on relays that have received the newer publish — no explicit deletion is required for rotation.

> **Note**: Addressable replacement is relay-local. A relay that missed the newer publish, or that later receives a replay of the older valid event, may still serve the stale version for that `d` slot. This is a residual failure mode: clients MUST treat the event with the highest `created_at` for a given `(kind, pubkey, d)` tuple as authoritative, and SHOULD discard older events for the same address. If multiple events for the same `(kind, pubkey, d)` tuple share the highest `created_at`, clients MUST use the lexicographically smallest `id` as a deterministic tiebreaker and discard the others. Implementers SHOULD publish rotation events to all relays in the KeyPackage's `relays` tag to minimize the window in which stale copies remain reachable.

#### Rotating KeyPackages

Clients SHOULD rotate KeyPackages by publishing a new `kind:30443` event with the same `d` tag value when:

- **Successfully joining a group**: After processing a `Welcome` message, publish a fresh KeyPackage under the same `d` identifier to ensure a valid KeyPackage remains available for future invitations.
- **Proactive rotation**: Periodically replace KeyPackages to limit exposure of any single `init_key`.

Rotation does NOT require sending a NIP-09 deletion event. The new event replaces the old one on relays that have received it.

#### Completely Removing a KeyPackage

Clients MAY send a [NIP-09](https://github.com/nostr-protocol/nips/blob/master/09.md) deletion event to explicitly remove a `kind:30443` event from relays when they want to signal that a KeyPackage slot is permanently decommissioned (e.g., retiring a device). This is distinct from rotation — use deletion only when you do not intend to publish a replacement under the same `d` identifier.

#### When NOT to Rotate KeyPackages

Clients MUST NOT rotate KeyPackages when:

- **Welcome processing fails**: If you cannot process a Welcome message (e.g., signing key was generated on another device), retain the existing KeyPackage so the inviter can retry or use a different approach.
- **Show clear errors**: Display user-friendly error messages explaining the issue.

> **Note**: For direct device-to-device KeyPackage sharing (QR code, Wi-Fi, Bluetooth, NFC), relay lifecycle considerations don't apply as the KeyPackage is transferred directly rather than stored on relays.
...
- **Last resort KeyPackages**: The private init_key MUST be retained until a replacement last_resort KeyPackage for the same `d` identifier has been published to all relays listed in the previous KeyPackage's `relays` tag and either:
  - the client has confirmation from each relay that it accepted the replacement publish, OR
  - a conservative grace window has elapsed after publication. Implementations that rely on a grace window SHOULD use at least 24 hours and MUST document their chosen duration.

  Only after one of these conditions is met MAY the old init_key be deleted.
...
Earlier versions of the Marmot specification used `kind:443` (a regular, non-replaceable event) for KeyPackages. The current specification uses `kind:30443` (an addressable event), which allows relay-native rotation via the `d` tag without requiring explicit NIP-09 deletion events.

#### What changed

| Property | kind:443 (legacy) | kind:30443 (current) |
|---|---|---|
| Event type | Regular | Addressable |
| Rotation mechanism | NIP-09 deletion + republish | Publish new event with same `d` tag |
| Relay deduplication | None (multiple versions accumulate) | Automatic per `(kind, pubkey, d)` triple |
| `d` tag | Not present | Required (random 32-byte hex) |

#### Consuming legacy kind:443 events

`kind:30443` is the canonical KeyPackage format defined by this specification. During any migration window where legacy `kind:443` KeyPackages must still interoperate with `kind:30443`, clients SHOULD query for both kinds simultaneously:
...
When both `kind:443` and `kind:30443` events are present for the same user, clients MUST prefer valid `kind:30443` events. If no valid `kind:30443` is available, clients MAY fall back to the freshest valid `kind:443` event. Within `kind:443` events, clients MUST prefer the event with the highest `created_at`. Deployment-specific cutover dates and rollout plans are recorded outside this MIP; see [docs/migrations/2026-05-01-kind-30443-cutover.md](docs/migrations/2026-05-01-kind-30443-cutover.md) for the current Marmot migration timeline.

#### Publishing

Clients that need interoperability with legacy peers SHOULD dual-publish equivalent KeyPackages as both `kind:30443` and `kind:443` during an active migration window. The legacy `kind:443` event SHOULD carry the same KeyPackage content and the same applicable tags as the `kind:30443` event, except for the `d` tag, which `kind:443` does not support. After the deployment-specific cutover, clients SHOULD publish only `kind:30443` KeyPackage events; see the migration guide for the Marmot project cutover date.

> **Note**: Because `kind:443` events have no `d` tag, they cannot be replaced in place — only deleted via NIP-09. Once equivalent `kind:30443` replacements are available and the migration window has ended, deployments SHOULD send NIP-09 deletion events for legacy `kind:443` KeyPackages.

#### Validation differences

`kind:443` events do not have a `d` tag. When processing legacy events:

- The `d` tag MUST NOT be required for `kind:443` events
- The `i` tag (`KeyPackageRef`) MAY be absent; compute it from event content if missing (see [Backward Compatibility](#backward-compatibility) in the KeyPackageRef section)
- All other validation rules (credential, encoding, ciphersuite) apply equally
...
2. **KeyPackages** (`kind: 30443`): Advertise your ability to join groups (via relays or direct sharing)
...
- Only include custom/non-default proposal types (like `0x000a` for self_remove, which Marmot implementations MUST include)
- Consider default extensions and proposal types as implicitly supported by all MLS implementations
