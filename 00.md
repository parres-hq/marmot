...
KeyPackage events are Nostr regular events. Implementations MUST NOT assume relays will keep only one KeyPackage per tag value, or that deletion requests will be applied consistently across relays. Relay-side deletion is a cleanup mechanism, not a correctness guarantee.
...
### KeyPackage Selection from Relays

When querying relays for KeyPackages, implementations MAY receive multiple events for the same user. Inviters MUST treat relays as an untrusted cache and select candidates client-side.

#### Candidate Validation

Before using a relay-fetched KeyPackage, inviters MUST reject candidates that fail any of the following checks:

- Invalid Nostr signature
- MLS credential identity does not exactly match the event `pubkey`
- Missing or invalid required Marmot tags
- `i` tag does not match the computed `KeyPackageRef` when the tag is present
- A valid same-author deletion request ([NIP-09](https://github.com/nostr-protocol/nips/blob/master/09.md)) references the event

#### Candidate Selection

If more than one valid KeyPackage remains for the same user, inviters MUST select as follows:

1. Prefer non-`last_resort` KeyPackages over `last_resort` KeyPackages.
2. Within the same class, prefer the candidate with the greatest `created_at` value.
3. If `created_at` values are equal, prefer the candidate with the lexicographically lowest event id.

Implementations MUST NOT depend on relay-specific policies such as "one event per tag" for correctness.
...
- **Creating new KeyPackages**: Publish deletion requests for superseded non-`last_resort` KeyPackages on the same relay set
...
Because relays may continue serving stale events after replacement or deletion, implementations SHOULD keep the private `init_key` material for superseded relay-published KeyPackages until either:

- The KeyPackage has been successfully used to process a Welcome, or
- The implementation has received successful deletion acknowledgements from each relay where that KeyPackage was published
