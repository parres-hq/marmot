...
- **`relays`**: **Required**. Relay URLs where this KeyPackage is published. MUST contain at least one valid WebSocket URL (ws:// or wss://). Used for KeyPackage discovery and deletion.
...
- **`-`**: Optional. Ensures only the author can publish this event (see [NIP-70](https://github.com/nostr-protocol/nips/blob/master/70.md)). Note: many popular relays reject events with the protected tag, so implementations SHOULD omit it unless publishing to relays known to support NIP-70.
