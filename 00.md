...
The hash function used to compute `KeyPackageRef` MUST be the hash function defined by the KeyPackage's ciphersuite, as specified in [RFC 9420 Section 17.1](https://www.rfc-editor.org/rfc/rfc9420.html#section-17.1). **All clients MUST use the ciphersuite's hash function** to ensure interoperability — using any other hash algorithm will produce incompatible `i` tag values.
...
const refHashInput = tlsSerialize({
  label: "MLS 1.0 KeyPackage Reference",
  value: tlsSerialized,
});
